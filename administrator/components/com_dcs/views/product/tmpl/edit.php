...
<script type="text/javascript">
	...
	window.addEvent('domready', function() {
	    ...
	    $$('.info').addEvent('click', function(){
	    	...
	    	Array.each(info, function(el){
				if(el.checked)
					checkLocation = true;
			});
	
	    	if(checkLocation)
	    		$('info-container').removeClass('invalid');
	        else
	        	$('info-container').addClass('invalid');
	    ...
	    $$('.info').addEvent('click', function(){
			if(this.checked)
			{
				...
				NewOption.inject($('jform_info'));
			}
			else
			{
				removeOption($('jform_info'), this.get('value'));
			}
		});
	});
...
<form action="<?php echo JRoute::_('index.php?option=com_dcs&layout=edit&id='.(int) $this->item->id); ?>" method="post" name="adminForm" id="product-form" class="form-validate" enctype="multipart/form-data">
	<div class="width-60 fltlft">
		<fieldset class="adminform">
			...
			<ul class="adminformlist">
					...
					<?php if($this->item->images && JEUtil::is_serialized($this->item->images)): ?>
						<div style="float:right; background: #FAFAFA; padding: 5px;">
							...
							<table cellpadding="0" cellspacing="1">
								...
								<tr>
									<td>
										<a href="../images/dcs_addresses/upload/<?php echo $img; ?>" class="modal">
											<img src="../images/dcs_addresses/upload/<?php echo $img; ?>" style="width: 100px; margin: 0;" />
										</a>
									</td>
									<td valign="top">
				...
				<li><?php echo $this->form->getLabel('product_info'); ?>
				<?php echo $this->form->getInput('product_info'); ?></li>
