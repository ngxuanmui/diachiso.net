 ...
 * @since		1.6
 */
class DcsModelProduct extends JModelAdmin
{
	/**
	...
	protected function canDelete($record)
	{
		...
		if ($record->catid) {
			return $user->authorise('core.delete', 'com_dcs.category.'.(int) $record->catid);
		} else {
	...
	protected function canEditState($record)
	{
		...
		if ($record->catid) {
			return $user->authorise('core.edit.state', 'com_dcs.category.'.(int) $record->catid);
		} else {
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Product', $prefix = 'DcsTable', $config = array())
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_dcs.product', 'product', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_dcs.edit.product.data', array());
	...
	function save($data)
	{
		...
		if($saveResult)
		{
			...
			//save image
			$query = "UPDATE #__dcs_addresses SET images = '".serialize($images)."' WHERE id = " . (int) $record->id;
			$db->setQuery($query);
	...
	{
		$db = JFactory::getDbo();
		$query = "SELECT id, images FROM #__dcs_addresses ORDER BY id DESC";
		$db->setQuery($query);
		...
		foreach ($result as $obj)
		{
				...
				//update
				$query = "UPDATE #__dcs_addresses SET images = '$images' WHERE id = " . (int) $obj->id;
				$db->setQuery($query);
