 ...
 * @since		1.6
 */
class JFormFieldDefaultInfo extends JFormFieldList
{
	/**
	 * The form field type.
	 *
	...
	public function getOptions()
	{
		...
		if(!$id)
		{
			...
			//get value
			$query = "SELECT * FROM #__dcs_addresses WHERE id = '$id'";
			$db->setQuery($query);
		...
		$query = "	SELECT i.info_id As value, IF(i.info_id = '*', 'Toàn quốc', info.name) As text
					FROM #__dcs_address_info i
					LEFT JOIN #__dcs_info info ON info.id = i.info_id
					JOIN #__dcs_addresses p ON i.product_id = p.id
